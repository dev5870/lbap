@extends('layouts.default', ['title' => __('title.menu.page')])
@section('content')
    ...
    <div class="flex-fill w-100 card table-responsive users">
        <table class="my-0 table table-striped users">
            ...
            <tbody>
                ...
                @include('includes.delete-modal', ['id' => 'deleteModal_'.$item->id, 'action' => Route('admin.page.destroy', $item)])
                @endforeach
            </tbody>
        </table>
    </div>
    <div class="paginationBlock">
        {{ $pages->appends(request()->query())->links('includes.pagination') }}
    </div>
@endsection
