@extends('layouts.default', ['title' => __('title.notification.many')])
@section('content')
    <div class="mb-2 mb-xl-3 d-flex justify-content-between">
        <div class="">
            <h3>{{ __('title.notification.many') }}</h3>
        </div>
        <div class="topButton">
    ...
    <div class="flex-fill w-100 card table-responsive users">
        <table class="my-0 table table-striped users">
            ...
            <tr>
                ...
                <th>{{ __('title.notification.text') }}</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $item->id }}</td>
                    <td>{{ $item->text }}</td>
                    <td>{{ \App\Enums\NotificationStatus::$list[$item->status] }}</td>
                ...
                @include('includes.delete-modal', ['id' => 'deleteModal_'.$item->id, 'action' => Route('admin.notification.destroy', $item)])
                @endforeach
            </tbody>
        </table>
    </div>
    <div class="paginationBlock">
        {{ $notifications->appends(request()->query())->links('includes.pagination') }}
    </div>
@endsection
